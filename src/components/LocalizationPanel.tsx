    ...
    <section className="panel">
      ...
      {sample ? (
        <>
          ...
          <div className="metric-row">
            <span>Kalman estimate</span>
            <strong>{formatPosition(sample.kalmanEstimatedPosition)}</strong>
          </div>
          ...
          <div className="metric-row">
            ...
            <strong>{metrics.kalmanCurrentError.toFixed(3)}</strong>
          </div>
          ...
          <div className="metric-row">
            <span>Kalman RMSE</span>
            <strong>{metrics.kalmanRmse.toFixed(3)}</strong>
          </div>
        </>
      ) : (
        <p className="panel-note">
          Run the simulation to generate GNSS-inspired noisy measurements,
          beacon range observations, least-squares estimates, Kalman estimates,
      ...
      <p className="panel-note">
        ...
        estimate applies simple temporal smoothing, the range LS estimate solves
        an unweighted nonlinear parametric least-squares problem from
        beacon-style range observations, and the Kalman estimate applies a
        constant-velocity prediction/update filter over time.
      </p>
    </section>
  );
}