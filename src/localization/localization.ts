...
export function buildLocalizationSamples(
    ...
    let kalmanState: KalmanFilterState | null = null;
    ...
    for (let step = 0; step < path.length; step++) {
        ...
        kalmanState =
            kalmanState === null
                ? createInitialKalmanState(measuredPosition)
                : updateKalmanFilter(kalmanState, measuredPosition);

        const kalmanEstimatedPosition = getKalmanPosition(kalmanState);
        ...
        const kalmanError = distance(truePosition, kalmanEstimatedPosition);
...
export function calculateLocalizationMetrics(
    ...
    if (activeSamples.length === 0) {
        return {
            ...
            rangeRmse: 0,
            sampleCount: 0,
            kalmanCurrentError: 0,
            kalmanAverageError: 0,
            kalmanRmse: 0,
        };
    }
    ...
    const kalmanCurrentError =
        activeSamples[activeSamples.length - 1].kalmanError;
    ...
    const kalmanErrorSum = activeSamples.reduce(
        (sum, sample) => sum + sample.kalmanError,
        0
    );

    const squaredKalmanErrorSum = activeSamples.reduce(
    ...
    return {
        ...
        kalmanCurrentError,
        kalmanAverageError: kalmanErrorSum / activeSamples.length,
        kalmanRmse: Math.sqrt(squaredKalmanErrorSum / activeSamples.length),
    };
}
